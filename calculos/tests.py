...
class VigaServiceTests(TestCase):
    ...
    def test_dimensionamento_viga_cenario_tipico(self):
        print("\n=> Executando teste para o dimensionamento de viga...")
        dados_viga = {
        ...
        self.assertEqual(resultado['combinacao_final'], '3 Ø 20')
        self.assertAlmostEqual(float(resultado['As_final_cm2']), 9.43, places=2)
        print("   Teste da viga concluído com sucesso!")
...
class SapataServiceTests(TestCase):
    ...
    def test_dimensionamento_sapata_cenario_tipico(self):
        ...
        # 1. ARRANGE: Preparamos os dados de entrada
        dados_sapata = {
        ...
        # 2. ACT: Executamos a função
        resultado = sapata_service.dimensionar_sapata(**dados_sapata)

        # 3. ASSERT: Verificamos os resultados chave
        self.assertEqual(resultado['status'], 'Sucesso')
