...
def dimensionar_pilar(b_mm, h_mm, l_m, lig_topo, lig_base, f_ck, f_yk, N_Ed_kN, M0_Ed_kNm, c_nom_mm, phi_ef):
    ...
    passos.append({"titulo": "3.1. Esbelteza Limite (λ_lim)", "formula": r"n = \frac{N_{Ed}}{A_c×f_{cd}} \, ; \, \lambda_{lim} = \frac{20 × A ×B × C}{\sqrt{n}}", "calculo": f"n = {N_Ed_N:.0f} / (({b_mm:.0f} x {h_mm:.0f}) x {f_cd_mpa:.2f}) = {n:.3f}<br>A = 1 / (1 + 0.2 x {phi_ef:.1f}) = {A:.3f}<br>λ_lim = (20 x {A:.3f} x {B:.1f} x {C:.1f}) / √n = {lambda_lim:.2f}"})
    ...
    resultado = {
        'status': 'Sucesso', 'mensagem': 'Cálculo efetuado com sucesso.', 
